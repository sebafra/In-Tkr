...
function defineFormsAction() {
	$('#finalUsersForm').attr("action",
			SERVER_URL + "/api/finalUser/saveWithPicture");

	$('#finalUsersImportForm').attr("action",
			SERVER_URL + "/api/finalUser/importCSVFile");
...
function searchUsers() {
	...
	var url = SERVER_URL + "/api/finalUser/getAll?json="
...
function formFinalUserCheckData() {
	...
	if ($.trim($("#inputDateTime").val()) == "") {
		...
		if ($('#inputPicture')[0].files[0].size > (PICTURE_MAX_SIZE * 1024)) {
			formFinalUserMessage = "La imagen no debe superar los "
					+ PICTURE_MAX_SIZE + "Kb";
			checkOK = false;
		}
...
function formFinalUserImportCheckSubmission() {
	...
	if (!formFinalUserImportSubmitted)
		return;
...
function getAllTrackers() {
	var url = SERVER_URL + "/api/tracker/getAll?json={entityId:%22"
			+ entityIdUrl + "%22}";
	$.getJSON(url, function(msg) {
		if (msg.status == "ok") {
...
function deleteUser() {
	...
	var deleteUrl = SERVER_URL + "/api/finalUser/remove?json={id:%22" + id
			+ "%22}";
...
function fillUsersList(i) {
	...
	$("#usersList").append(
			"<tr id='user" + i + "' style='cursor:pointer'><td>"
					+ finalUsers[i].dni + "</td><td>" + finalUsers[i].firstName
					+ "</td><td>" + finalUsers[i].lastName + "</td><td>" + imei
					+ "</td></tr>");
}
			...
			.on(
					...
					function() {
						...
						if (inputEmpty === 0) {
							if (checkEmail() === true) {
								if (id === "") {
									var file = "{datetime:%22" + datetime
											+ "%22,entityId:%22" + entityIdUrl
											+ "%22,userName:%22" + userName
											+ "%22,userPassword:%22"
											+ userPassword
											+ "%22,firstName:%22" + firstName
											+ "%22,lastName:%22" + lastName
											+ "%22,email:%22" + email
											+ "%22,phones:%22" + phones
											+ "%22,notes:%22" + notes
											+ "%22,dni:%22" + dni + "%22}";
...
function imeiListReload() {
	...
	for (var i = 0; i < trackers.length; i++) {
		if (trackers[i].finalUserId == undefined)
			$("#inputImei").append(
					"<option value=" + trackers[i].id + ">" + trackers[i].imei
							+ "</option>");
	}
...
function imeiList() {
	...
	for (var i = 0; i < trackersAll.length; i++) {
		if (trackersAll[i].finalUserId == undefined)
			$("#inputImei").append(
					"<option value=" + trackersAll[i].id + ">"
							+ trackersAll[i].imei + "</option>");
	}
}

function imeiListOld() {
	var imeiItems = SERVER_URL + "/api/tracker/getAll?json={entityId:%22"
			+ entityIdUrl + "%22}";
		...
		if (msg.status == "ok") {
			...
			for (var i = 0; i < msg.data.trackers.length; i++) {
				if (msg.data.trackers[i].finalUserId == undefined)
					$("#inputImei").append(
							"<option value=" + msg.data.trackers[i].id + ">"
									+ msg.data.trackers[i].imei + "</option>");
			}
		} else {
			showAlert("msg", "danger", msg.error.message);
		}
...
function addImeiToList(trackerId) {
	if (trackerId == undefined)
		return;
	if (trackersAll == undefined)
		return;

	for (var i = 0; i < trackersAll.length; i++) {
		if (trackersAll[i].id == trackerId) {
			$("#inputImei").prepend(
					"<option selected=true value=" + trackersAll[i].id + ">"
							+ trackersAll[i].imei + "</option>");
