...
function authenticate(userName, password, userType) {
	$("#btnLoginSubmit").html(
			"<img src='img/ajax-loader.gif' alt='placeholder+image'>");
	var loginFile = SERVER_URL + "/api/user/login?json={name:%22" + userName
			+ "%22,password:%22" + password + "%22,type:%22" + userType
			+ "%22}";
...
function createNewEntity() {
	...
	var userTel = $("#entityTel").val();
	checkInput("#entityTel");
	...
	if (inputEmpty === 0) {
		if (checkPassword(userPassword, userPassword2) === true) {
			...
			var entityFile = SERVER_URL + "/api/entity/create?json={name:%22"
					+ name + "%22,userName:%22" + userName
					+ "%22,userPassword:%22" + userPassword + "%22,tel:%22"
					+ userTel + "%22,email:%22" + userEmail + "%22}";
			$.getJSON(entityFile, function(msg) {
				if (msg.status == "ok") {
					showAlert("msg-modal", "success",
							"Entidad creada con &eacute;xito");
			...
			$("#saveForm").html("Guardar");
			showAlert("msg-modal", "danger",
					"Las contrase&ntilde;as no coinciden"); //inputEmpty
		}
	} else {
		$("#saveForm").html("Guardar");
