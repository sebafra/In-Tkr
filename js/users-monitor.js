...
function showOperators() {
	var loginFile = SERVER_URL + "/api/operatorUser/getAll?json={entityId:%22"
			+ entityIdUrl + "%22}";
...
function fillOperatorsList(i) {
	$("#operatorsList").append(
			"<tr id='operator" + i + "' style='cursor:pointer'><td>"
					+ operatorUsers[i].firstName + "</td><td>"
					+ operatorUsers[i].lastName + "</td><td>"
					+ operatorUsers[i].userName + "</td></tr>");
}
	...
	$("#saveOperator")
			.on(
					...
					function() {
						...
						var firstName = $.trim($("#inputOperatorFirstName")
								.val());
						var lastName = $
								.trim($("#inputOperatorLastName").val());
						var userName = $
								.trim($("#inputOperatorUsername").val());
						var userPassword = $.trim($("#inputOperatorPassword")
								.val());
						var userPassword2 = $.trim($("#inputOperatorPassword2")
								.val());
						if (id == "") {
							var file = SERVER_URL
									+ "/api/operatorUser/create?json={firstName:%22"
									+ firstName + "%22,lastName:%22" + lastName
									+ "%22, userName:%22" + userName
									+ "%22,userPassword:%22" + userPassword
									+ "%22,entityId:%22" + entityIdUrl + "%22}";
											...
											.getJSON(
													...
													function(result) {
														if (result.status == "ok") {
															showAlert("msg",
																	"success",
																	"Operador creado con &eacute;xito");
															showOperators();
															resetOperatorsForm();
														} else {
															showAlert("msg",
							...
							var file = SERVER_URL
									+ "/api/operatorUser/update?json={firstName:%22"
									+ firstName + "%22,lastName:%22" + lastName
									+ "%22, userName:%22" + userName
									+ "%22,userPassword:%22" + userPassword
											...
											function(result) {
												if (result.status == "ok") {
													showAlert("msg", "success",
															"Operador modificado con &eacute;xito");
													showOperators();
												} else {
													showAlert("msg", "danger",
															"Ha ocurrido un error");
...
function deleteOperator() {
	$("#deleteOperator").on(
			...
			function() {
				...
				console.log(id);
				var file = SERVER_URL + "/api/operatorUser/remove?json={id:%22"
						+ id + "%22}";
				if (id !== "") {
					$.getJSON(file, function(result) {
						if (result.status == "ok") {
							showAlert("msg", "success",
									"Operador borrado con &eacute;xito");
							showOperators();
							resetOperatorsForm();
						} else {
							showAlert("msg", "danger", "Ha ocurrido un error");
